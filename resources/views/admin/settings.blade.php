    ...
    <div class="grid grid-cols-1 gap-6 xl:grid-cols-2 mt-5">
        <div class="panel">
            <div>
                ...
                <form action="{{route('ToggleMaintenance')}}" class="mt-5" method="POST">
                    @csrf
                    <label for="date" class="mb-2 w-1/3 ltr:mr-2 rtl:ml-2" style="font-size:15px">Estimated Up
                        Time:</label>
                    <input class="form-input flex-1 mb-2" @if($status->is_enabled) disabled @endif id="date"
                    type="datetime-local" name="estimated_up_time" required>
                    <label for="note" class="mb-2 w-1/3 ltr:mr-2 rtl:ml-2" style="font-size:15px">Reason:</label>
                    <textarea @if($status->is_enabled) disabled @endif class="form-input flex-1 mb-2" name="reason" id="note" cols="30" rows="10"
                        required></textarea>
                    @if($status->is_enabled)
                    <button type="submit" class="btn  btn-outline-success   mt-3 gap-2" style="width:100%"><svg
                        ...
                        </svg>LIFT MAINTENANCE </button>
                    @else
                    <button type="submit" class="btn  btn-outline-danger   mt-3 gap-2" style="width:100%"><svg
                            width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
                            class="h-5 w-5 shrink-0 ltr:mr-2 rtl:ml-2">
                            <path
                                d="M3.46447 20.5355C4.92893 22 7.28595 22 12 22C16.714 22 19.0711 22 20.5355 20.5355C22 19.0711 22 16.714 22 12C22 11.6585 22 11.4878 21.9848 11.3142C21.9142 10.5049 21.586 9.71257 21.0637 9.09034C20.9516 8.95687 20.828 8.83317 20.5806 8.58578L15.4142 3.41944C15.1668 3.17206 15.0431 3.04835 14.9097 2.93631C14.2874 2.414 13.4951 2.08581 12.6858 2.01515C12.5122 2 12.3415 2 12 2C7.28595 2 4.92893 2 3.46447 3.46447C2 4.92893 2 7.28595 2 12C2 16.714 2 19.0711 3.46447 20.5355Z"
