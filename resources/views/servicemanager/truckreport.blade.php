        ...
        <div class="panel">
            <div class="table-responsive">
                <table class="orders-table table table-bordered">
                    ...
                    <tbody>
                        @foreach ($reports as $num => $item)
                        <tr>
                            <td>{{$num + 1}}</td>
                            <td style="text-align: center">
                                @if ($item->urgent === 0)
                                <span class="badge badge-outline-success">Urgent</span>
                                @else
                                <span class="badge badge-outline-warning">Report</span>
                                @endif
                            </td>
                            <td>{{$item->driver->name}}</td>
                            <td>{{$item->plate}}</td>
                            <td style="width: 30%">{{$item->report}}</td>
                            <td style="width: 20%; text-align: center">{{
                                \Carbon\Carbon::parse($item->created_at)->format('F j, Y h:m A')
                                }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                ...
                <div class="dataTable-bottom mt-5">
                    ...
                    <nav class="dataTable-pagination">
                        <ul class="dataTable-pagination-list">
                            ...
                            @else
                            <li class="pager"><a
                                    href="{{ request()->fullUrlWithQuery(['page' => $reports->currentPage() - 1]) }}"><svg
