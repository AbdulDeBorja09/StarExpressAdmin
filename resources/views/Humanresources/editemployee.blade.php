...
<div class="animate__animated p-6" :class="[$store.app.animation]">
    <ol class="flex font-semibold text-gray-500 dark:text-white-dark">
        ...
        <li class="before:px-1.5 before:content-['/']">
            <a href="javascript:;"
                class="text-black hover:text-black/70 dark:text-white-light dark:hover:text-white-light/70">Edit
                Employee</a>
        </li>
    </ol>
    ...
    <div class="mt-5">
        <div class="flex flex-col gap-2.5 xl:flex-row">
            <form action="{{route('submitemployeeedit')}}" method="POST" enctype="multipart/form-data"
                        ...
                        <div class="w-full lg:w-1/2">
                            ...
                            <div class="mt-4 flex items-center">
                                ...
                                <select id="branch" name="branch_id" class="form-select flex-1"
                                    ...
                                    <option value="{{ $items->id }}" @if($items->id == $items->branch_id) selected
                                        @endif>
                                        {{ $items->country }}, {{ $items->branch }}
                                    </option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="mt-4 flex items-center">
                                ...
                                <select id="type" name="type" class="form-select flex-1"
                                    style="text-transform: capitalize">
                                    <option value="hr" @if($items->type == 'hr') selected @endif>Human Resources
                                    </option>
                                    <option value="servicemanager" @if($items->type == 'servicemanager') selected
                                        @endif>Service Manager</option>
                                    <option value="accountant" @if($items->type == 'accountant') selected
