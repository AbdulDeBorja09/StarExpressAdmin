...
<div class="animate__animated p-6" :class="[$store.app.animation]">
    ...
    <div class="panel mt-5">
        <form action="{{route('submitdriveredit')}}" method="POST">
            @csrf
            ...
            <div class="mt-8 px-4">
                <div class="flex flex-col justify-between lg:flex-row">
                    <div class="mb-6 w-full lg:w-1/2 ltr:lg:mr-6 rtl:lg:ml-6">
                        ...
                        $fname = $nameParts[0] ?? '';
                        $mname = (isset($nameParts[1]) && str_contains($nameParts[1], '.')) ? $nameParts[1] : '';
                        $lname = implode(' ', array_slice($nameParts, $mname ? 2 : 1));
                        @endphp
                        <input type="hidden" name="id" value="{{$item->id}}">
                        <div class="mt-4 flex items-center">
                            <label for="fname" class="mb-0 w-1/3 ltr:mr-2 rtl:ml-2">Frist Name</label>
                            <input id="fname" type="text" name="fname" value=" {{ $fname }}" class="form-input flex-1"
                                placeholder="Enter First Name">
                        </div>
                        <div class="mt-4 flex items-center">
                            <label for="mname" class="mb-0 w-1/3 ltr:mr-2 rtl:ml-2">Middle Name</label>
                            <input id="mname" type="text" name="mname" value=" {{ $mname  }}" class="form-input flex-1"
                                placeholder="Enter Middle Name">
                        </div>
                        <div class="mt-4 flex items-center">
                            <label for="lname" class="mb-0 w-1/3 ltr:mr-2 rtl:ml-2">Last Name</label>
                            <input id="lname" type="text" name="lname" value="{{$lname}}" class="form-input flex-1"
                        ...
                        <div class="mt-4 flex items-center">
                            <label for="contact" class="mb-0 w-1/3 ltr:mr-2 rtl:ml-2">Contact
                                Number</label>
                            <input id="contact" type="number" name="contact" value="{{$item->phone}}"
                    ...
                    <div class="w-full lg:w-1/2">
                        ...
                        <div class="mt-4 flex items-center">
                            ...
                            <select id="branch" name="branch_id" class="form-select flex-1"
                                ...
                                <option value="{{ $items->id }}" @if($items->id == $items->branch_id) selected @endif>
